...
Serves three OCR vision-language models via OpenAI-compatible APIs on a GPU server. Tested on NVIDIA DGX Spark (GB10 Blackwell, 128GB unified memory).
...
This script:
1. Auto-detects your server IP (Tailscale/LAN) and prompts for confirmation
...
Set `DGX_HOST` to override the server IP:
```bash
